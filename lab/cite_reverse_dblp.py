...
def process_entry(entry, output_file):
    ...
    with open(output_file, "a", encoding="utf-8") as f_out:
        if dblp_entry:
            ...
            comment = ''
            f_out.write(comment + entry + "\n\n")
            return "not_found", info["cite_key"]
...
def main(input_file="test.bib", output_file="test_new.bib"):
    ...
    for i, entry in enumerate(entries):
        ...
        try:
            status, cite_key = process_entry(entry, output_file)
            if status in summary and cite_key:
                summary[status].append(cite_key)
            elif cite_key:
                summary["invalid"].append(cite_key)
        except Exception as e:
            print(f"处理条目时出错: {e}")
            # 出错时增加延迟
    ...
    print("\n处理结果统计：")
    print(f"  ✓ 成功匹配 DBLP 条目: {len(summary['matched'])} 条 -> {summary['matched']}")
    print(f"  ~ 仅找到 arXiv 版本: {len(summary['arxiv_only'])} 条 -> {summary['arxiv_only']}")
    print(f"  ✗ 未找到 DBLP 匹配: {len(summary['not_found'])} 条 -> {summary['not_found']}")
    if summary["invalid"]:
        print(f"  ! 条目解析失败: {len(summary['invalid'])} 条 -> {summary['invalid']}")
